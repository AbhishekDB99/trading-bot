...
function sessionDate() {
  // Returns the Date object for the target session date (or today if null)
...
function filterSessionCandles(candles) {
  ...
  // For historical dates use market close; for today use now
  const isToday = !CONFIG.TARGET_DATE;
  const closeMs = isToday ? Date.now() : marketCloseUTC().getTime();
...
async function run() {
  ...
  const modeLabel = CONFIG.TARGET_DATE
    ? `Historical  : ${CONFIG.TARGET_DATE}  (set TARGET_DATE: null for live)`
    : `Current IST  : ${formatIST(new Date())}`;
  ...
  // In historical mode ALL candles are completed. In live mode, the last
  // candle may be mid-formation so we exclude it from direction but keep
  // it for volatility (range spike on the current bar is intentional).

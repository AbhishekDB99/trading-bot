...
Scanning ${STOCKS.length} stocks — BUY signals only`,
  );
  ...
  for (const symbol of STOCKS) {
    ...
    // Only send BUY signals to Telegram — skip WAIT, AVOID, errors
    if (result.signal === "BUY") {
  ...
  if (buys.length) {
    ...
    summaryLines.push("🔍 No BUY signals today.");
    summaryLines.push(
      "All stocks are either extended, at weak support, or in downtrend.",
    );
    summaryLines.push("Wait for better setups — cash is a position.");
  }
  summaryLines.push("");
  summaryLines.push("⚠️ Always verify on chart. Use a stop loss.");

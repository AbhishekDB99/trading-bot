...
Fetching data...`);
  ...
  // ── Pass 1: diagnostic — count why signals are rejected on last day ──
  console.log("  Running filter diagnostics on most recent day...");
  const lastDayIdx   = totalCandles - 1;
  const lastNifty    = niftyAll.slice(0, lastDayIdx + 1);
  const lastRegime   = detectRegime(lastNifty);
  console.log(`  Last day regime: ${lastRegime}`);
  let diagCounts = { noData:0, tooFewCandles:0, noSignalDate:0, volatile:0,
    notNearHigh:0, notUpClose:0, weakRS:0, lowVolume:0, stopTooWide:0,
    stopTooTight:0, targetTooSmall:0, dedupe:0, passed:0 };

  for (const symbol of STOCKS) {
    const stockAll = allCandles[symbol];
    if (!stockAll) { diagCounts.noData++; continue; }
    const stockDayIdx = stockAll.findIndex((c) => Math.abs(c.time - niftyAll[lastDayIdx].time) < 86400 * 2);
    if (stockDayIdx < 30) { diagCounts.tooFewCandles++; continue; }
    const sc = stockAll.slice(0, stockDayIdx + 1);
    if (lastRegime === "VOLATILE") { diagCounts.volatile++; continue; }
    if (lastRegime !== "BULLISH") continue;

    const cp = sc[sc.length-1].close;
    const pc = sc[sc.length-2]?.close;
    const avgV = avg(sc.slice(-21,-1).map(c=>c.volume));
    const lastV = sc[sc.length-1].volume;
    const rh = Math.max(...sc.slice(-CONFIG.HIGH_LOOKBACK_DAYS,-1).map(c=>c.high));
    const dfh = pct(cp, rh);
    const ss = sc[sc.length-1-CONFIG.REL_STRENGTH_DAYS]?.close;
    const ns = lastNifty[lastNifty.length-1-CONFIG.REL_STRENGTH_DAYS]?.close;
    const sr = ss ? pct(cp,ss) : null;
    const nr = ns ? pct(lastNifty[lastNifty.length-1].close, ns) : null;
    const rs = sr!=null&&nr!=null ? sr-nr : null;
    const atr = calcATR14(sc);
    const stop = atr ? cp-(atr*1.5) : null;
    const rp = stop ? Math.abs(pct(cp,stop)) : null;
    const tgt = stop ? cp+(cp-stop)*CONFIG.MIN_RR : null;
    const tp = tgt ? pct(tgt,cp) : null;

    if (dfh < -CONFIG.HIGH_PROXIMITY_PCT*100) { diagCounts.notNearHigh++; continue; }
    if (pc && cp <= pc) { diagCounts.notUpClose++; continue; }
    if (!rs || rs < CONFIG.REL_STRENGTH_MIN) { diagCounts.weakRS++; console.log(`    RS reject: ${symbol.replace(".NS","")} RS=${rs?.toFixed(1)}% (need ${CONFIG.REL_STRENGTH_MIN}%)`); continue; }
    if (lastV < avgV*CONFIG.BREAKOUT_VOLUME_X) { diagCounts.lowVolume++; console.log(`    Vol reject: ${symbol.replace(".NS","")} vol=${(lastV/avgV).toFixed(2)}x (need ${CONFIG.BREAKOUT_VOLUME_X}x)`); continue; }
    if (!rp || rp > 8) { diagCounts.stopTooWide++; continue; }
    if (rp < 1) { diagCounts.stopTooTight++; continue; }
    if (!tp || tp < 5) { diagCounts.targetTooSmall++; continue; }
    diagCounts.passed++;
    console.log(`    ✓ ${symbol.replace(".NS","")} would pass: RS=${rs?.toFixed(1)}% vol=${(lastV/avgV).toFixed(2)}x risk=${rp?.toFixed(1)}%`);
  }
  console.log("  Rejection reasons:", JSON.stringify(diagCounts));
  console.log(div);

...
const TELEGRAM = {
  ...
  CHAT_ID:   "5975650526",                 // personal or group ID
};
...
const CONFIG = {
  ...
  HIGH_LOOKBACK_DAYS:     30,   // near 30-day high (tighter window)
  HIGH_PROXIMITY_PCT:     0.02, // within 2% of recent high
  REL_STRENGTH_DAYS:      20,
  REL_STRENGTH_MIN:       5,    // must outperform NIFTY by 5%+
  BREAKOUT_VOLUME_X:      2.0,  // need 2× avg volume for confirmation
...
function runBreakoutScanner(candles, symbol, niftyCandles) {
  ...
  // ── Volume: today must be 2× avg of prior 20 days ──
  const avgVol  = avg(candles.slice(-21, -1).map((c) => c.volume));
  const lastVol = candles[candles.length - 1].volume;
  const volRatio = lastVol / avgVol;

  // ── Near recent high (within 2%, excluding today) ──
  ...
  const stopPrice  = atr ? currentPrice - (atr * 1.5) : null;
  const riskPct    = stopPrice ? Math.abs(pct(currentPrice, stopPrice)) : null;
  const riskUnit   = stopPrice ? currentPrice - stopPrice : null;
  const target     = riskUnit  ? currentPrice + riskUnit * CONFIG.MIN_RR : null;
  const targetPct  = target    ? pct(target, currentPrice) : null;

  // ── Decision ──
  let signal, reason, warning = null;

  if (distFromHigh < -CONFIG.HIGH_PROXIMITY_PCT * 100) {
    signal = "WAIT";
    reason = `Price is ${fmt(Math.abs(distFromHigh), 1)}% below ${CONFIG.HIGH_LOOKBACK_DAYS}-day high — not in breakout zone yet.`;
  } else if (prevClose && currentPrice <= prevClose) {
    signal = "WAIT";
    reason = `Near high but today's close is not up — no breakout momentum. Wait for a strong up-close day.`;
  } else if (!relStrength || relStrength < CONFIG.REL_STRENGTH_MIN) {
    signal = "WAIT";
    reason = `Near high but RS vs NIFTY is only ${relStrength != null ? fmt(relStrength, 1) + "%" : "unknown"} — need ${CONFIG.REL_STRENGTH_MIN}%+ outperformance.`;
  } else if (volRatio < CONFIG.BREAKOUT_VOLUME_X) {
    signal = "WAIT";
    reason = `Near high + good RS but volume only ${fmt(volRatio, 2)}× avg — need ${CONFIG.BREAKOUT_VOLUME_X}× for conviction.`;
  } else if (!stopPrice || riskPct > 8) {
    signal = "WAIT";
    reason = `Conditions met but stop is ${fmt(riskPct, 1)}% away (ATR too wide) — risk too large for this setup.`;
  } else if (riskPct < 1) {
    signal = "WAIT";
    reason = `Stop is only ${fmt(riskPct, 1)}% away — too tight, likely to get stopped on noise.`;
  } else if (targetPct < 5) {
    signal = "WAIT";
    reason = `Target upside is only ${fmt(targetPct, 1)}% — not worth the risk. Need 5%+ potential.`;
  } else {
    signal = "BUY";
    reason = `Breaking near ${CONFIG.HIGH_LOOKBACK_DAYS}-day high (${fmtINR(recentHigh)}) | Vol: ${fmt(volRatio, 1)}x | RS vs NIFTY: +${fmt(relStrength, 1)}% | ATR stop: ${fmt(riskPct, 1)}% risk`;
    if (riskPct > 5) warning = `Risk is ${fmt(riskPct, 1)}% — size down accordingly.`;
  }
  ...
  return {
    ...
    stopPrice:   stopPrice ? +stopPrice.toFixed(2) : null,
    target:      target    ? +target.toFixed(2)    : null,
    riskPct:     riskPct   ? +riskPct.toFixed(2)   : null,
    targetPct:   targetPct ? +targetPct.toFixed(2) : null,

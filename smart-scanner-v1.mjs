...
Slope : ${regimeData.ema20Slope}%  |  ATR Ratio : ${regimeData.atrRatio}×
━━━━━━━━━━━━━━━━━━━━
📌 ${regimeData.reason}
...
function buyMessageSR(symbol, r) {
...
async function run() {
  ...
  // BEARISH — send stay cash, no scanning
  if (regimeData.regime === "BEARISH") {
    console.log("  🔴 BEARISH regime — sending stay cash alert. No scan.");
  ...
  for (const symbol of STOCKS) {
    process.stdout.write(`  ${regime === "BULLISH" ? "🚀" : "📊"} ${symbol.padEnd(18)} ... `);
  ...
  console.log();
  console.log(hdr);
  console.log(`  REGIME: ${regime}  |  ALGO: ${algoName[regime]}`);
  console.log(hdr);

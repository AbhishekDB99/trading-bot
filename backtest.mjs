...
function runSRScanner(candles) {
  ...
  // Allow price within NEAR_LEVEL_PCT above OR up to 1.5% below support
  // Daily candles: price closing slightly below support = testing the level
  const atSupport  = distFromSupp <= CONFIG.NEAR_LEVEL_PCT * 100 && distFromSupp >= -1.5;
...
Fetching data...`);
  ...
  for (const symbol of STOCKS) {
    ...
    const atSupport = dist <= CONFIG.NEAR_LEVEL_PCT * 100 && dist >= -1.5;
  ...
  for (let dayIdx = startIdx; dayIdx < totalCandles; dayIdx++) {
    ...
    // UNKNOWN = not enough EMA warmup data — treat same as SIDEWAYS
    const effectiveRegime = regime === "UNKNOWN" ? "SIDEWAYS" : regime;
